...
- Update the package, Plugin, compatibility registry, bilingual documentation, and candidate release material to V7.12.0 while advancing the Codex CLI anchor to 0.156.0. The fixed eleven-release window now spans 0.156.0 through 0.152.0; 0.151.0 leaves the active window.
- Local package/runtime validation, SessionEnd recovery regressions and quick-status measurements are recorded in the release reports. The experimental LIGHT shortcut is withheld after the comparison; original LIGHT instructions remain. The 20 quick-status pairs belong to historical payload `948c`; the current payload has not been remeasured.
- Fixed-profile host-routing combined coverage retains 13 original evidence records and adds three targeted follow-ups: `HOST_FINAL_REPORT` records 16 observed actual independent tasks, 16 PASS and none missing. It is neither a single-cohort pass-rate claim nor an independent router trace. The Windows PowerShell Hook launcher fix uses a pinned `cmd.exe /D /V:OFF /S /C` wrapper; managed installation exited 0 while native lifecycle results and execution conditions are recorded in the validation report.
- The installer updates only `hooks.json` and preserves trusted hashes in `hooks.state`; the new wrapper makes the current ten managed Hooks appear modified. After upgrading, review and trust them in the normal Codex `/hooks` UI without a bypass; passing full status does not mean trusted or lifecycle-passed.
- Final host lifecycle, remote CI, artifact provenance and publication require separate readback.
- Install and verify the account-level `seal_worker.py` as a managed sibling; reject a missing entry before process creation. A fresh native task passes all five event types and current-head sealing, with the local elevated-setup limitation and temporary official fallback explicitly retained.
...
- A complete recovery query returns exit code 0 while retaining its `STALE` warning; partial or unknown queries still return 1. The POSIX entry rejects an invalid explicitly selected interpreter and validates each automatically discovered interpreter, matching the Windows entry.
- Preserve stable Windows/Ubuntu required-check names for the protected branch. Aggregate checks depend on all four Python validations and block the build if the matrix is unsuccessful.
...
- Candidate behavior, host and installed-task acceptance completed with explicit limits; net cost savings are not established. See [validation](docs/releases/v7.6.0/VALIDATION_REPORT.md) and [capability index](docs/CAPABILITY_INDEX.md).
