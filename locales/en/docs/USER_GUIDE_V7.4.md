...
V7.4.2 preserves one root-task weighted DelegationBudget for Reviewer, Explorer, and Worker. Weights are `1/2/4/8`; LIGHT, STANDARD, and STRICT provide `4/16/32` total units.
...
V7.4.2 does not create a root-task ledger automatically. Without task-scoped explicit activation, only the automatic model ceiling is active and the unified budget gate must not be reported as passed.
...
V7.4.2 supports Codex CLI 0.153.2 and the ten preceding stable releases exactly as frozen in the compatibility registry. Installation is complete only after Plugin readback reports `installed=true`, `enabled=true`, and `version=7.4.2`, and the schema-3 host snapshot is `HOST_COMPATIBLE`.
