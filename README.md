...
V7.4.5 将稳定兼容窗口推进到 Codex CLI 0.153.3；每个 Release 标题均由受约束的双语 `release_name` 生成，通用候选标题会被拒绝。发布工作流仍仅创建 Draft，不会自动公开。
...
| 发行包 | 适用界面 | 下载 |
| --- | --- | --- |
| `Codex-Skills-V7.4.5-zh-CN.zip` | 简体中文 | [下载中文安装包](https://github.com/JimmyVGDY/codex-long-term-assistant-skills/releases/download/v7.4.5/Codex-Skills-V7.4.5-zh-CN.zip) |
...
生命周期可形成 `TURN_OPENED -> SUBAGENT_STARTED -> SUBAGENT_STOPPED -> TASK_COMPLETED` 事件序列；会话结束后由 `SessionEnd` 进入延迟封印流程。V7.4.5 只证明派发前策略符合批准档位与 Terra High 上限，不读取、不推断、不保存或导出宿主实际使用的模型与推理强度。

## 兼容矩阵

| 环境或模式 | 当前定位 | 已有验证层级 | 边界 |
| --- | --- | --- | --- |
| Windows 原生 Codex CLI 0.153.3 + Plugin | 当前实机锚点 | V7.4.5 账户级事务安装、Plugin 读回、宿主兼容快照与三方 payload 摘要一致 | 宿主模型身份不属于验收信息；实际卸载/回滚和父子 Agent 生命周期旅程未在本次发布中执行 |
| Windows + 11 个固定 Codex 稳定版 | 隔离矩阵 | 0.153.3 新增单元的 CLI、Plugin 往返、合成 Hook 与官方制品校验已通过 | 完整 11 版本矩阵尚须在候选提交 CI 读回 |
...
V7.4.5 的冻结窗口为 `0.153.3`、`0.153.2`、`0.153.1`、`0.153.0`、`0.152.1`、`0.152.0`、`0.151.0`、`0.150.1`、`0.150.0`、`0.149.1`、`0.149.0`。补丁版本独立计数；未来版、预发布版和其他窗口外版本不自动接纳。
...
3. 仅当 Plugin 读回 `installed=true`、`enabled=true`、`version=7.4.5`，schema 3 宿主状态为 `HOST_COMPATIBLE`，且旧领域 Skill 不再发现时，升级状态才成立。
...
- [版本记录](CHANGELOG.md) · [V7.4.5 发行说明](docs/releases/v7.4.5/RELEASE_NOTES.md)
