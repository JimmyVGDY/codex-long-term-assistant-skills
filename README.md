...
V7.10.0 提供基础安装即用和按需增强运行时：普通工程任务可直接开始，Hook、长期状态、预算和受控写入只在需要时接入。
...
V7.6.2 已恢复消息与 Stop 非阻断边界；V7.7.0 新增 Operation v2；V7.7.1 将兼容窗口前移到 Codex CLI 0.154.0。V7.10.0 保留这些安全边界，并将基础 Skill 与可选增强运行时解耦；门禁仍默认关闭，能力档位不代表业务语义正确或权限已授予。
...
解压后的源码树和发行 ZIP 都提供同一组 `cp-assistant` 入口；Windows 使用 `scripts\\cp-assistant.ps1`，POSIX 使用 `scripts/cp-assistant.sh`。`help` 与 `install-base` 不需要本包 Python，其他管理命令会明确提示 Python 3.11+ 依赖，不会伪造 Plugin 状态。

| 日常路径 | 示例请求 | 入口动作 |
| --- | --- | --- |
| 修 Bug | “修复失败验证并运行最小相关测试” | 先描述问题；需要状态时运行 `status` / `doctor` |
| 做功能 | “实现这个功能并按风险验证” | 直接描述目标；增强能力按需安装 |
| 审查 | “审查当前分支的兼容与安全风险” | 直接描述审查范围；独立 Reviewer 按风险启用 |
| 继续任务 | “继续上次任务，说明做到哪里和下一步” | 运行只读 `resume`，不自动恢复安装事务 |
...
| 发行包 | 适用界面 | 下载 |
| --- | --- | --- |
| `Codex-Skills-V7.10.0-zh-CN.zip` | 简体中文 | [下载中文安装包](https://github.com/JimmyVGDY/codex-long-term-assistant-skills/releases/download/v7.10.0/Codex-Skills-V7.10.0-zh-CN.zip) |
...
| --- | --- | --- | --- |
| Windows 原生 Codex CLI 0.154.0 + Plugin | 当前实机锚点 | 目标版本的账户安装、`verify` 与项目外新 CLI 任务将在 S8/S9 独立读回 | Desktop 重启后的新会话未验证 |
...
V7.10.0 窗口继续为 `0.154.0`、`0.153.4`、`0.153.3`、`0.153.2`、`0.153.1`、`0.153.0`、`0.152.1`、`0.152.0`、`0.151.0`、`0.150.1`、`0.150.0`。未来版、预发布版和窗口外版本不会自动接纳。
...
- [版本记录](CHANGELOG.md) · [V7.10.0 发行说明](docs/releases/v7.10.0/RELEASE_NOTES.md)
...
```shell
gh attestation verify Codex-Skills-V7.10.0-zh-CN.zip --repo OWNER/REPOSITORY
```
