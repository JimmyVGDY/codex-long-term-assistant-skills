...
V7.7.1 将冻结兼容窗口前移到 Codex CLI 0.154.0 与此前十个稳定发行版，并保留 V7.7.0 的 Operation v2 写前协议、默认关闭门禁和失败关闭边界。
...
Codex CLI 0.154.0 的 Plugin 刷新、Hook schema 与 `apply_patch` 成功结果合同已分别核验；上游移除的 `codex mcp-server` 不在本仓库调用链中。V7.7.1 继续保留 Operation v2、规范 `apply_patch/tool_input.command/tool_use_id` 对账和 Luna/Terra 自动派发上限。
...
| 发行包 | 适用界面 | 下载 |
| --- | --- | --- |
| `Codex-Skills-V7.7.1-zh-CN.zip` | 简体中文 | [下载中文安装包](https://github.com/JimmyVGDY/codex-long-term-assistant-skills/releases/download/v7.7.1/Codex-Skills-V7.7.1-zh-CN.zip) |
...
V7.7.1 的窗口为 `0.154.0`、`0.153.4`、`0.153.3`、`0.153.2`、`0.153.1`、`0.153.0`、`0.152.1`、`0.152.0`、`0.151.0`、`0.150.1`、`0.150.0`。未来版、预发布版和其他窗口外版本不会自动接纳；发行工作流只创建草稿。
...
3. 仅当 Plugin 读回 `installed=true`、`enabled=true`、`version=7.7.1`，schema 3 宿主状态为 `HOST_COMPATIBLE`，且旧领域 Skill 不再发现时，升级状态才成立。
...
- [版本记录](CHANGELOG.md) · [V7.7.1 发行说明](docs/releases/v7.7.1/RELEASE_NOTES.md)
...
```shell
gh attestation verify Codex-Skills-V7.7.1-zh-CN.zip --repo OWNER/REPOSITORY
```
