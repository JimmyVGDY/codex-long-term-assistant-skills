#!/usr/bin/env python3
"""中文：构建并验证字节级可复现的 V7.11.1 语言发行包。

English: Build and verify byte-reproducible V7.11.1 locale-specific archives.
"""
...
def build_release(output: Path, locale: str) -> Dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="cp-v720-build-") as temporary_dir:
...
def reproducible_build(output: Path, witness: Path, locale: str) -> Dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="cp-v720-repro-") as temporary:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="V7.11.1 deterministic bilingual release builder")
