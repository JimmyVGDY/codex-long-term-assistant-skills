#!/usr/bin/env python3
"""Build and verify a byte-reproducible V6.6 release archive."""
...
def build_release(root: Path, output: Path) -> Dict[str, Any]:
    ...
    if manifest.get("version") != "6.6.0":
        raise BuildError("manifest version must be 6.6.0")
    if output == root or root in output.parents:
        raise BuildError("release output must stay outside the package source")
    write_payload_manifest(root, "codex-cross-project-engineering-assistant", "6.6.0")
    ...
    package_root = "Codex-Skills-V6.6"
    try:
...
def verify_release(archive_path: Path) -> Dict[str, Any]:
    ...
    with zipfile.ZipFile(archive_path, "r") as archive:
        ...
        if any(not name.startswith("Codex-Skills-V6.6/") for name in names):
...
def reproducible_build(root: Path, output: Path, witness: Path) -> Dict[str, Any]:
    with tempfile.TemporaryDirectory(prefix="cp-v66-repro-") as temporary:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="V6.6 deterministic release builder")
