...
if manifest.get('version')!='7.1.0': errors.append('manifest 版本不是 7.1.0')
if manifest.get('user_skills_target')!='$HOME/.agents/skills': errors.append('账户 Skill 目标不是 $HOME/.agents/skills')
plugin=json.loads((ROOT/'.codex-plugin'/'plugin.json').read_text(encoding='utf-8'))
if plugin.get('version')!='7.1.0': errors.append('Plugin 版本不一致')
if manifest.get('default_locale')!='zh-CN' or manifest.get('supported_locales')!=['zh-CN','en']:
    errors.append('双语发行声明无效')
for previous in ('6.1.0','6.2.0','6.3.0','6.4.0','6.5.0','6.6.0','6.6.1','7.0.0'):
    if previous not in manifest.get('upgrade_from',[]): errors.append('缺少 %s -> V7.1.0 升级声明' % previous)
...
for release_script in ('build-release.py','lifecycle-acceptance.py','model-gate-acceptance.py','seal-worker.py','event-archive.py','release-attestation.py','verify-release.py','payload-integrity.py','validate-v71.py'):
    if not (ROOT/'scripts'/release_script).is_file(): errors.append('缺少 V7.1 发布证明脚本: '+release_script)
payload=json.loads((ROOT/'PLUGIN_PAYLOAD_MANIFEST.json').read_text(encoding='utf-8'))
if payload.get('version')!='7.1.0' or payload.get('file_count',0)<1: errors.append('Plugin payload manifest 无效')
...
for primary in ('README.md','CHANGELOG.md','global/AGENTS.md','docs/USER_GUIDE_V7.1.md',
...
if errors:
    ...
    raise SystemExit(1)
print('[OK] V7.1.0 语义校验通过：Codex CLI 0.152.1、账户工具事务化安装、四主领域、双语发行、10 Skills、7 Reviewers、Plugin/Hooks、payload 身份、模型上限和受控演进边界一致')
