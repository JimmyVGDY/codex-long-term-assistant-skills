...
if manifest.get('version')!='6.4.0': errors.append('manifest 版本不是 6.4.0')
if manifest.get('user_skills_target')!='$HOME/.agents/skills': errors.append('账户 Skill 目标不是 $HOME/.agents/skills')
plugin=json.loads((ROOT/'.codex-plugin'/'plugin.json').read_text(encoding='utf-8'))
if plugin.get('version')!='6.4.0': errors.append('Plugin 版本不一致')
for previous in ('6.1.0','6.2.0','6.3.0'):
    if previous not in manifest.get('upgrade_from',[]): errors.append('缺少 %s -> V6.4 升级声明' % previous)
...
for release_script in ('build-release.py','lifecycle-acceptance.py','release-attestation.py','verify-release.py','payload-integrity.py','validate-v64.py'):
    if not (ROOT/'scripts'/release_script).is_file(): errors.append('缺少 V6.4 发布证明脚本: '+release_script)
payload=json.loads((ROOT/'PLUGIN_PAYLOAD_MANIFEST.json').read_text(encoding='utf-8'))
if payload.get('version')!='6.4.0' or payload.get('file_count',0)<1: errors.append('Plugin payload manifest 无效')
...
if errors:
    ...
    raise SystemExit(1)
print('[OK] V6.4 语义校验通过：10 Skills、Plugin/Hooks、payload 身份、事件恢复、发布证明、模型上限、受控演进边界和中性语言门禁一致')
