...
if manifest.get('version')!='6.6.1': errors.append('manifest 版本不是 6.6.1')
if manifest.get('user_skills_target')!='$HOME/.agents/skills': errors.append('账户 Skill 目标不是 $HOME/.agents/skills')
plugin=json.loads((ROOT/'.codex-plugin'/'plugin.json').read_text(encoding='utf-8'))
if plugin.get('version')!='6.6.1': errors.append('Plugin 版本不一致')
if manifest.get('default_locale')!='zh-CN' or manifest.get('supported_locales')!=['zh-CN','en']:
    errors.append('双语发行声明无效')
for previous in ('6.1.0','6.2.0','6.3.0','6.4.0','6.5.0','6.6.0'):
    if previous not in manifest.get('upgrade_from',[]): errors.append('缺少 %s -> V6.6.1 升级声明' % previous)
...
if payload.get('version')!='6.6.1' or payload.get('file_count',0)<1: errors.append('Plugin payload manifest 无效')
...
for primary in ('README.md','global/AGENTS.md','docs/USER_GUIDE_V6.6.1.md',
...
if errors:
    ...
    raise SystemExit(1)
print('[OK] V6.6.1 语义校验通过：双语发行、10 Skills、7 Reviewers、Plugin/Hooks、payload 身份、延迟封印、模型上限、受控演进边界和中性语言门禁一致')
