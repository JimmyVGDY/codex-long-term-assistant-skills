...
if manifest.get('version')!='7.13.3': errors.append('manifest 版本不是 7.13.3')
if manifest.get('user_skills_target')!='$HOME/.agents/skills': errors.append('账户 Skill 目标不是 $HOME/.agents/skills')
plugin=json.loads((ROOT/'.codex-plugin'/'plugin.json').read_text(encoding='utf-8'))
if plugin.get('version')!='7.13.3': errors.append('Plugin 版本不一致')
...
for previous in ('6.1.0','6.2.0','6.3.0','6.4.0','6.5.0','6.6.0','7.0.0','7.1.0','7.2.0','7.3.0','7.4.0','7.4.1','7.4.2','7.4.3','7.4.4','7.4.5','7.4.6','7.5.0','7.5.1','7.6.0','7.6.1','7.6.2','7.7.0','7.7.1','7.9.2','7.10.0','7.13.0'):
    if previous not in manifest.get('upgrade_from',[]): errors.append('缺少 %s -> V7.13.3 升级声明' % previous)
registry=load_registry(ROOT/'config'/'codex-compatibility-v1.json','7.13.3')
...
if payload.get('version')!='7.13.3' or payload.get('file_count',0)<1: errors.append('Plugin payload manifest 无效')
...
for primary in ('docs/releases/v7.13.3/RELEASE_NOTES',
                'docs/releases/v7.13.3/VALIDATION_REPORT',
                'docs/releases/v7.13.3/AUDIT_REPORT'):
...
if errors:
    ...
    raise SystemExit(1)
print('[OK] V7.13.3 语义校验通过：Desktop 范围、AUTO 注册、onboarding/1、Operation v2 与历史解释器边界一致')
