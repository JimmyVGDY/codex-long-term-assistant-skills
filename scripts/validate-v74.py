...
def validate_package():
    with tempfile.TemporaryDirectory(prefix='cp-v74-validation-') as temporary:
        ...
        run([sys.executable,str(ROOT/'scripts'/'payload-integrity.py'),'verify','--root',str(ROOT),
             '--manifest',str(ROOT/'PLUGIN_PAYLOAD_MANIFEST.json'),'--package','codex-cross-project-engineering-assistant','--version','7.7.0'],validation_env)
...
result={
 'ok':True,'evidence_scope':'package-only','version':'7.7.0','skill_count':10,'reviewer_count':7,
