...
if args[:2] == ['plugin','add']:
    ...
    version=os.environ.get('FAKE_PLUGIN_VERSION','7.5.1')
...
if args == ['plugin','list','--json']:
    ...
    installed=[]
    if state.exists():
        installed=[{'pluginId':'codex-cross-project-engineering-assistant@cp-assistant-local','name':'codex-cross-project-engineering-assistant','marketplaceName':'cp-assistant-local','version':os.environ.get('FAKE_PLUGIN_VERSION','7.5.1'),'installed':True,'enabled':True,'installPolicy':'AVAILABLE','authPolicy':'ON_INSTALL'}]
...
""",encoding='utf-8')
    ...
    def test_plugin_tools_fail_closed_when_state_or_bound_cache_is_unavailable(self):
        ...
        cache=self.codex/'plugins'/'cache'/'cp-assistant-local'/'codex-cross-project-engineering-assistant'/'7.5.1'
    ...
    def test_plugin_install_rejects_wrong_registered_version(self):
        ...
        self.assertIn('version=7.5.1',result.stderr)
        self.assertFalse((self.codex/'cp-assistant-v6-transaction.json').exists())
    ...
    def test_crash_journal_recovers_and_status_is_json(self):
        ...
        self.assertEqual('7.5.1',status['version'])
