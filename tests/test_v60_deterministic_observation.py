...
class V60DeterministicObservationTests(unittest.TestCase):
    ...
    def test_hook_blocks_sol_and_allows_terra_high(self):
        ...
        r=subprocess.run([sys.executable,str(hook)],input=json.dumps(bad),text=True,capture_output=True,env=env,timeout=10)
        ...
        r=subprocess.run([sys.executable,str(hook)],input=json.dumps(good),text=True,capture_output=True,env=env,timeout=10)
    ...
    def test_proposal_lifecycle_requires_accept_task_baseline_validation_and_close(self):
        ...
        view=reg.decide(view.proposal.proposal_id,DecisionType.ACCEPT,'Jimmy','确认该提案证据充分，但所有实施动作仍需独立任务与验证。')
        self.assertEqual(ProposalStatus.ACCEPTED,view.current_status)
        view=reg.link_implementation(view.proposal.proposal_id,'Jimmy','TASK-IMPLEMENT-1','baseline-sha')
        self.assertEqual(ProposalStatus.IMPLEMENTATION_LINKED,view.current_status)
        view=reg.record_validation(view.proposal.proposal_id,'Jimmy','commit-sha',['evidence://targeted-test-pass'])
        self.assertEqual(ProposalStatus.VALIDATION_RECORDED,view.current_status)
        view=reg.close(view.proposal.proposal_id,'Jimmy','PASS')
