...
平台、任务或项目上限更低时采用更低值。不得通过拆分等价调用、重复命名或重建台账规避预算。
...
生产、真实数据、权限安全、资金、库存和不可逆迁移默认采用 `system-readonly`。可写父会话且无 sandbox denied 证据时，只能报告 `logical-readonly`。

受控写入探针只允许在一次性临时 Git 仓库中运行；禁止在正式项目、生产目录、账户主目录或真实数据目录测试。
