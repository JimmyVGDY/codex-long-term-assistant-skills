...
账户 Session、分布式任务状态、全局锁、限流计数、幂等记录、业务状态和多实例共享缓存不得只存在进程内。

进程内缓存只适用于可丢失、可重建、无需实例一致、有大小上限和 TTL 的数据。必须评估多 Worker 内存倍增和状态不一致。
...
未获明确授权不做大版本升级、不升级无关依赖、不在生产直接执行未经审查的 `pip install -U`。
