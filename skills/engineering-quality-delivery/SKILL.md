...
涉及新增功能、修复、重构或共享实现改动时，修改前读取 [组件与模块复用](references/component-module-reuse.md)，按任务规模完成已有能力定位与适用性判断；只读和无行为变化任务不触发。

能力索引按[统一流程](references/capability-index-workflow.md)完成以下适用动作：

- **修改前**：已绑定Profile且有核验过的runtime入口时，先限定查询确认索引状态；不能凭任务简单或工作区未见索引推断缺失。已有本任务可靠的缺失证据且不适用初扫时可直接查源码。首次共享公开接口或多使用方规则变更需限定初扫；`INDEX_MISSING`不豁免该触发，LIGHT档只缩小范围。
- **结束前**：本次协调者使变更路径失效，限定扫描已检查的变更源文件，以及查询后已核实并实际采用的过期候选源文件，再读回；候选未改动也需刷新已过期定位。仅invalidate不算更新完成。子Agent交回发现，由协调者合并。
- 无索引的简单局部修复直接检索源码；无关任务不创建索引。不为该流程额外启动Agent或全仓扫描。
...
8. 宿主提供完整任务反馈绑定且已有工程验证时，通过 `scripts/evolution.py validate-task` 执行该验证并保留结构化结果；主协调者在最终回答前调用 `finalize-task`，明确终态、失败分类、返修次数和路由判断。计数从证据机械提取，不保存命令或输出正文。没有完整绑定或有效证据时保留 UNKNOWN，不额外运行无意义验证；普通任务不触发完整演进分析。
