...
description: >-
  日志、Metrics 指标、分布式 Trace、Profiling、告警和发布/配置变更事件分析时使用。覆盖本地文件、开发测试、远程非生产和生产只读场景；负责多证据源关联和根因验证，不因分析可观测性数据自动获得修改、采集、清理、重启或生产写权限。
---
...
用于本地或上传日志、压缩日志包、应用标准输出、Metrics、分布式 Trace、已有 Profiling 产物、告警、发布和配置变更事件，以及 Docker、Kubernetes、systemd、Java、Python、数据库、中间件、网关、网络和存储相关可观测性数据的结构化分析。
...
4. 先建立证据源清单和统一时间线，再分别分析日志、指标、Trace、Profile、告警和变更事件，最后进行多证据源关联并形成根因假设。
5. 严格区分已确认、高概率、推测和未验证；日志相关性不能直接写成因果关系。
6. 输出关键证据时只保留最小必要片段并脱敏，不复制完整 Token、Cookie、密钥、身份证、手机号、地址或大段业务数据。
7. 远程和生产只读分析必须限制时间窗、行数、文件范围、查询基数、Trace 数量、Profiling 采集时长和命令成本；禁止无限 `tail -f`、无边界扫描、Redis `KEYS *`、高消耗全表查询和未经授权的在线 Profiling。
...
- 指标分析：`assets/templates/METRICS_ANALYSIS.template.md`
- Trace 分析：`assets/templates/TRACE_ANALYSIS.template.md`
- 多证据源关联：`assets/templates/OBSERVABILITY_CORRELATION.template.md`

## 核心原则

> 先确定范围、时间和证据源，再关联日志、指标、Trace、Profile、告警与变更；先区分现象与根因，再提出验证；只读不等于无风险，相关性不等于因果性。
