...
- 按任务串行登记并在账本锁内核对等价生命周期证明；身份核验不可用、冲突或登记消失时拒绝继续。
- 统一仓库外状态与受管根目录保护，保留未创建合法子目标的支持。
- 冻结 V3 默认、GPT-5.6 兼容与 GPT-6 资格门槛不变；完整包、安装和真实原生准入分别验收。
...
- 非断言子测试异常保留既有 ERROR 分类与固定异常类型，不扩展报告格式或记录敏感正文。
- 安装器集成测试使用 120 秒进程等待，覆盖多次宿主探针与文件事务；功能断言、生产命令和 Hook 时限不变。
- 延续 GPT-6 兼容支持与冻结 V3 默认；原生派发、模型资格和默认切换仍须单独验收。V7.13.0 标签保留为未完成发行验收的候选，本补丁的完整验证、制品和公开发行另行读回。

## 7.13.0 - 2026-09-24

- 增加六型号、十八个 Low/Medium/High 评测组合；V4 生产候选为 GPT-6 Luna、Sol、Astra 的九个组合，须先通过场景资格与收益证据，默认仍使用冻结 V3，旧任务按原策略恢复。
- 增加按场景核验的资格、收益和成本卡，比较提高思考强度与更换型号的收益；预算保留必要的后续复审与修复额度，并分别限制 Astra 累计尝试和在途并发。
- 增加 V4 根账本、V9/V6 审查、版本隔离观察、固定配对实验集，以及桌面任务名与生命周期 ID 的受限关联；未知回执不自动完成或退款。
- 产品边界限定 Codex Desktop，管理组件契约与真实原生验收分别记录；保留历史解释器与恢复格式，不建立独立 CLI 适配或验收轨道。
- 发行暂存区统一 Windows 启动脚本的 CRLF 换行，消除不同 Git 检出方式造成的包内容和校验和差异；源文件与冻结账本保持原字节。
- 本版本交付兼容支持阶段：默认继续使用冻结 V3，新模型仅开放明确的评测范围。代码、跨平台 CI、候选包和安装分别验证；原生预算链路与模型资格尚未通过，不启用 GPT-6 生产默认，也不宣称严格预算已生效。
...
- 更新包、Plugin、兼容注册表、双语文档与候选发行材料至 V7.12.0；将 Codex CLI 锚点推进到 0.156.0；按当前版加此前十个稳定版的既定规则，窗口为 0.156.0 至 0.152.0，0.151.0 移出活动窗口。
- 已记录本地整包与运行时验证、SessionEnd 恢复回归及快速状态测量。LIGHT 快捷路径经对照后暂不交付，保留原执行规则；20 对快速状态样本属于旧 payload `948c`，当前 payload 尚未重测。
- 固定 profile 的宿主路由组合覆盖为 13 个保留原始证据加 3 个定向补充任务，`HOST_FINAL_REPORT` 记录 16 个实际独立任务均通过、无缺失；不把它表述为单一 cohort 通过率或独立 router trace。Windows PowerShell 的 Hook 启动修复使用固定 `cmd.exe /D /V:OFF /S /C` 包装器；受管安装 exit 0，真实原生 Hook 生命周期结果及运行条件见验证报告。
...
- 补齐账户级 `seal_worker.py` 的受管安装与验证；Hook 使用同目录 worker，缺失入口在创建进程前拒绝。全新原生任务的五类事件和当前链头封存通过，保留本机 elevated 初始化限制与临时官方回退条件。
...
- 保留旧策略文件、评分解释器及账本/状态重放；各初始化入口明确接受新旧策略并核对根绑定。模型组合、权重、额度和最高可负担选择保持原值。已完成本机账户升级与原生只读加载验收，尚未公开发布。

- 适配官方稳定版 Codex CLI 0.155.0，保留当前版及此前十个稳定版窗口；单独核验命令、Plugin、Hook 与 apply_patch 源码契约，预发布版本仍失败关闭。

## 7.11.0 - 2026-09-17

本地候选；本次尚未公开发布。

- 独立 Reviewer 新增 Sol/Astra Low、Medium、High，形成十组合；默认从 Luna Low 起算，按复审预算和有效证据重算分值后一次派发，明确质量集合和成本代理。
- 新增冻结策略、V3 根预算、V8 复审/V5 结果与版本隔离校准；保留 V2/V7/V4 旧流程，防止重复许可、跨根身份、预算混算和静默降档。
- 通过原生回调关联工具调用、Agent 创建和启停；无任务名接口携带随机单次派发引用，仅存摘要，拒绝隐式猜配、错误引用和重复消费；不把缺失 agent_id 当作根调用者证明。V3 完成状态不能由手工 start/complete 替代，未知未启动原因不自动退款。
...
- 修正状态与诊断聚合、已安装版本/制品读回、查询稳定性与显式项目控制准备检查；受限或损坏状态返回结构化结果，恢复建议保留实际作用范围。
- 恢复查询复用安全读取、校验真实检查点引用和身份，并对总读取量、进程输出、超时与后代进程回收设限；不以旧 Markdown 指纹或抽样内容宣称当前验证有效。
- 统一入口保留调用者目录并减少重复 Python 启动；修复无 Python 的帮助与安装帮助边界。基准按真实样本重算统计并拒绝混合身份、无效数值和覆盖既有结果。
- 未完成事务的恢复建议优先于控制准备提示；诊断对读取错误变化和事务变化重新采样，并拒绝超大或损坏的事务记录。修复无关 Git 文本转换器导致恢复证据误报未知的问题；基准在输入目录不存在时拒绝启动采样。
- 完整恢复查询发现过期证据时保留 `STALE` 提示并返回退出码 0；部分或未知查询仍返回 1。POSIX 入口拒绝不合格的显式解释器，并逐一验证自动发现的解释器，与 Windows 入口保持一致。
- 为受保护主分支保留稳定的 Windows/Ubuntu 检查名；汇总检查依赖四组 Python 验证，任一未通过就阻止后续构建。

## 7.9.2 - 2026-09-14

- 补齐发行构建元数据并同步兼容注册表摘要；修复英文发布说明标题，继续交付默认能力注册表修复。V7.9.1 标签保留为未公开发布的失败候选。

## 7.9.1 - 2026-09-14

- 修复 Plugin 安装后能力注册表默认读取返回 UNREADABLE：在受管 runtime 中携带与权威配置一致的注册表，源码与安装布局分别定位；新增隔离安装、显式路径失败和损坏文件回归验证。
