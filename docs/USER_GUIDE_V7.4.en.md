...
V7.4.2 does not create a root-task ledger automatically. The unified budget is activated explicitly per task. Without both environment variables, the Hook still enforces the automatic model ceiling, but that task must not be reported as having passed the unified budget gate.
...
V7.4.2 supports Codex CLI 0.153.2 and the ten preceding stable releases exactly as frozen in `config/codex-compatibility-v1.json`. The local Marketplace manifest requires `interface.displayName`; future, prerelease, and other out-of-window hosts are not admitted automatically.

Installation is established only when Plugin readback reports `installed=true`, `enabled=true`, and `version=7.4.2`, and the schema-3 host snapshot is `HOST_COMPATIBLE`.
