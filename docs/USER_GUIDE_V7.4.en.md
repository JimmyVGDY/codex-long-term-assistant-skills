...
V7.4.5 does not create a root-task ledger automatically. The unified budget is activated explicitly per task. Without both environment variables, the Hook still enforces the automatic dispatch-profile ceiling, but that task must not be reported as having passed the unified budget gate.
...
## 5. Codex 0.153.3 scope

V7.4.5 supports Codex CLI 0.153.3 and the ten preceding stable releases exactly as frozen in `config/codex-compatibility-v1.json`. The upstream 0.153.3 changes concern Amazon Bedrock model selection and async-question guidance; they do not change the frozen Plugin/Hook contract or the automatic subagent policy, which remains limited to Luna/Terra profiles. The local Marketplace manifest requires `interface.displayName`; future, prerelease, and other out-of-window hosts are not admitted automatically.

Installation is established only when Plugin readback reports `installed=true`, `enabled=true`, and `version=7.4.5`, and the schema-3 host snapshot is `HOST_COMPATIBLE`.
