...
V7.7.0 uses a frozen registry for the Plugin, Marketplace, and PreToolUse/PostToolUse interfaces in Codex CLI 0.153.4 and the ten preceding stable releases. Plugin registration is established only when `codex plugin list --json` reads back `installed=true`, `enabled=true`, and `version=7.7.0`, and the schema-3 host snapshot is `HOST_COMPATIBLE`. Files present on disk do not establish installation or enablement.
