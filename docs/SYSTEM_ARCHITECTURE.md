...
> 状态：`active`。本页描述 V7.5.1 当前包的整体架构；旧版本设计与发行证据只用于历史追溯。
...
包版本是 V7.5.1；`TaskOutcomeEvent V3`、Evolution Policy 等名称是组件合同或数据格式标识，不代表安装了旧版软件。
...
日志、质量交付、独立复审、技术文档、长期记忆和受控演进作为支撑能力按阶段加载。详细边界见 [V7.5 领域 Skill 架构](V7_DOMAIN_SKILL_ARCHITECTURE.md) 和 [V7.5 Skill 触发矩阵](SKILL_TRIGGER_MATRIX.md)。
...
- 当前规范从[文档中心](README.md)进入，并明确标记 V7.5；
