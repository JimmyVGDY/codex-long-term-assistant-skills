...
> 状态：`active`。本页描述 V7.3.0 当前行为；Evolution 组件 Manifest 的 `5.1.0` 和默认策略 `v6.5-default-1` 是内部合同版本。
...
V7.3 按信号分别评估证据充足性：模型升级依赖实际模型覆盖，负面结果依赖已知终态覆盖，路由偏差依赖明确路由观察，Reviewer 收益依赖稳定身份与归因覆盖。某个信号证据不足时只阻断该信号，不无条件否决其他证据充分的候选。
