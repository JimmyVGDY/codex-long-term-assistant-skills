...
> 状态：`active`。本页适用于 V7.4.3；Evolution 组件 Manifest 仍使用 `5.1.0` 合同版本，默认策略为 `v6.5-default-1`，两者都不是当前包版本。
...
- 需要评估批准派发档位、Reviewer 组合或 Skill 路由是否合理。
...
证据充足性按信号分别判断：派发档位价值回归需要相邻批准档位的结果与单位成本样本，负面结果需要已知终态覆盖，路由偏差需要明确路由观察，Reviewer 收益需要可归因结果。一个信号缺少证据不会无条件阻断其他证据充分的信号。
