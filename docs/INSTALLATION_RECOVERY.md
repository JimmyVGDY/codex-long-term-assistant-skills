...
- 当前升级应读回已安装版本与 `to_version=6.6.1`；V6.6.0 升级路径必须被识别；
- schema 1 到 2 迁移；
- 新升级备份路径；
...
完成条件：Plugin 精确读回 `installed=true`、`enabled=true`、`version=6.6.1`，并且 10 个 Skill、7 个 Reviewer、6 个 Hook、延迟封印 worker、keyring 和 payload digest 全部通过。文件复制完成不构成 Plugin 成功状态。
...
python scripts\build-release.py verify --archive ..\Codex-Skills-V6.6.1-zh-CN.zip --locale zh-CN
python scripts\release-attestation.py verify --attestation ..\release-attestation-v6.6.1.json --artifact ..\Codex-Skills-V6.6.1-zh-CN.zip
```
