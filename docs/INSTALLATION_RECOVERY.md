...
- 目标宿主：Windows 原生 Codex CLI 0.152.1。
- Python：3.11 或更高版本。
- 推荐形态：账户级 Plugin。
- 可升级版本：7.1.0、7.0.0、6.6.1、6.6.0、6.5.0、6.4.0、6.3.0、6.2.0、6.1.0、6.0.0、5.1.0、5.0.0、4.2.0、4.1.0、4.0.0。
...
- 当前升级应读回已安装版本与 `to_version=7.2.0`；V7.1.0 升级路径必须被识别；
...
Codex 0.152.1 的本地 Marketplace manifest 不接受旧安装器写入的顶层 `owner/interface`。升级器会在备份后移除这两个已知受管字段，并保留其他未知外部字段；`codex plugin list --json` 恢复正常后才继续激活。

完成条件：Plugin 精确读回 `installed=true`、`enabled=true`、`version=7.2.0`，并且 10 个 Skill、7 个 Reviewer、6 个 Hook、延迟封印 worker、keyring 和 payload digest 全部通过。`java-backend-engineering`、`python-backend-ai-engineering`、`data-middleware-ai-infrastructure` 和此前废弃的 `vue-frontend-engineering` 不得残留；文件复制完成不构成 Plugin 成功状态。
...
python scripts\build-release.py verify --archive ..\Codex-Skills-V7.2.0-zh-CN.zip --locale zh-CN
python scripts\release-attestation.py verify --attestation ..\release-attestation-v7.2.0.json --artifact ..\Codex-Skills-V7.2.0-zh-CN.zip
```

`validate-package.py` 调用当前 `validate-v72.py`，检查执行前后的 Git index、受管与未跟踪文件内容、删除状态和链接类型；`--output` 只能写到仓库外。其 `routing_host_observation` 固定为 `NOT_EVALUATED`，不能替代真实宿主路由验收。
