# V7.2 Codex Configuration Guide

> Status: `active`. This page covers current V7.2 configuration only. Follow [installation and recovery](INSTALLATION_RECOVERY.en.md) for installation, upgrade, and recovery procedures.
...
V7.2 uses the Plugin and Marketplace interfaces in Codex CLI 0.152.1. Plugin registration is established only when `codex plugin list --json` reads back `installed=true`, `enabled=true`, and `version=7.2.0`. Files present on disk do not establish installation or enablement.
...
3. A new task can discover ten V7.2 Skills and seven Reviewers.
