...
- 补丁根因：安装状态有意保存全局 AGENTS 的受管区块哈希，但 V7.8.0 新增 inventory 时误用整文件 `tree_sha256` 比较，导致正确安装也稳定误报 `DRIFT`；内容和权限并未漂移。
- 修复只改变 inventory 的哈希投影与安装提交时的读回校验。标记外自有内容保持独立，受管区块唯一性、路径归属和重解析点拒绝保持失败关闭。
...
- 合并 V7.7.1 main 后必须保留 Codex 0.154.0 注册表、停用门禁 PostTool 对账、对应兼容测试、账户加载证据和完整历史；V7.8.1 重新执行受影响验证，不复用合并前计数。
