...
V7.4.4 示例标题：`V7.4.4 | 版本化发行标题与历史标题回填 / Versioned Release titles and historical title backfill`；实际标题以清单字段读回为准。
...
检查清单还需确认标题来源与约束、job output/环境变量传递、Draft-only、既有 Release 不覆盖、双语资产与 provenance、Codex CLI 0.153.2 兼容边界，以及历史标题回填的独立授权和线上读回。
