...
V7.6.0 English title segment: `Component reuse index and optional workflow gates`; the complete bilingual title is assembled from the two manifest values.
...
The checklist must also confirm title sources and constraints, job-output/environment-variable transport, Draft-only behavior, existing-Release no-overwrite behavior, bilingual assets and provenance, the Codex CLI 0.153.4 compatibility boundary, and independent authorization/readback for historical title backfill.
