...
本计划从 V7.8.1 开始实施。基础 Plugin 的目标是无需 Plugin 专属 Python、Profile、索引、台账或 Hook 即可安装并开始普通工程任务；增强能力在用户需要长期任务、索引、受控写入或硬预算时单独接入。

基础能力不等于放宽授权、降低项目测试要求，或绕过已启用的安全控制。安装、注册、当前任务加载和业务验收分别记录。

## 分层契约

| 组件 | 内容 | 依赖 | 用户可见结果 |
| --- | --- | --- | --- |
| base | 10 个 Skill 与用户入口元数据 | Codex Plugin 宿主 | 安装后可直接描述任务 |
| enhanced | Hook、Python runtime、账户级工具、Reviewer、全局受管规则 | Python 3.11+ 与已验证宿主契约 | 启用长期记忆、索引、预算和受控写入 |
| strict controls | 已启用门禁、Required 预算等 | enhanced 的等价执行控制 | 受控操作失败关闭 |

base 与 enhanced 使用同一 Skill 规则源，不能形成两套工程规范。增强组件不提供重复 Skill，避免宿主发现两份同名 Skill。
