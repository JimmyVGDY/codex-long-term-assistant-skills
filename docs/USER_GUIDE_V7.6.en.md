...
Use the [capability index](CAPABILITY_INDEX.en.md) for bounded initial scans and incremental updates. Recheck candidate source, semantic compatibility, and maintenance cost before reuse. Project gates are disabled by default. In V7.6.2, legacy GateTask and prepare/finish/check receipts do not grant native-write permission: enabled policies reject native writes safely, while unconfigured or disabled policies remain neutral. See the [acceptance protocol](COMPONENT_REUSE_ACCEPTANCE.en.md) and [release validation](releases/v7.6.2/VALIDATION_REPORT.en.md).
