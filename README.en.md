...
V7.4.6 advances the stable compatibility window to Codex CLI 0.153.4 and requires every Release title to be generated from a constrained bilingual `release_name`; generic candidate titles are rejected. The workflow remains draft-only and never publishes automatically.
...
| --- | --- | --- |
| `Codex-Skills-V7.4.6-zh-CN.zip` | Chinese | [Download zh-CN package](https://github.com/JimmyVGDY/codex-long-term-assistant-skills/releases/download/v7.4.6/Codex-Skills-V7.4.6-zh-CN.zip) |
| `Codex-Skills-V7.4.6-en.zip` | English | [Download English package](https://github.com/JimmyVGDY/codex-long-term-assistant-skills/releases/download/v7.4.6/Codex-Skills-V7.4.6-en.zip) |
...
| --- | --- | --- | --- |
| Native Windows Codex CLI 0.153.4 + Plugin | Current real-host anchor | V7.4.6 account-level transactional installation, Plugin readback, host compatibility snapshot, and three-way payload digest match passed | Host model identity is outside the acceptance contract; actual uninstall/rollback and a parent/child Agent lifecycle journey were not exercised by this release |
| Windows + eleven pinned stable Codex releases | Isolated matrix | CLI, Plugin round-trip, synthetic Hook, and official artifact checks pass for the new 0.153.4 cell | The complete eleven-version matrix awaits candidate-commit CI readback |
...
The V7.4.6 window is `0.153.4`, `0.153.3`, `0.153.2`, `0.153.1`, `0.153.0`, `0.152.1`, `0.152.0`, `0.151.0`, `0.150.1`, `0.150.0`, and `0.149.1`. Patch releases count independently; future, prerelease, and other out-of-window hosts are not admitted automatically.
...
3. The upgrade is established only when Plugin readback reports `installed=true`, `enabled=true`, and `version=7.4.6`, schema-3 host state is `HOST_COMPATIBLE`, and every Manifest-declared legacy Skill directory is absent.
...
- [Changelog](CHANGELOG.en.md) · [V7.4.6 release notes](docs/releases/v7.4.6/RELEASE_NOTES.en.md)
